...
export class MCTSNode {
  ...
  constructor(
    ...
    public moves: Action[],
    public parent: MCTSNode | undefined
  ) {
    this.parent = parent;
    this.visits = 0;
    this.totalScore = 0;
...
export function evaluateConnectPotential(
  state: GameState,
  connect: Connection
): number {
  ...
  const worthPenalty = Math.min(
    1,
    totalWorth(state) / (connectCost(connect.tier) * 5)
  );
  ...
  return (
    ...
    connect.dealChance *
    tonedDownDealSize *
    connect.priceQuality
  );
}

export function evaluateDealPotential(
  state: GameState,
...
function likelyBestMove(state: GameState): Action {
  ...
  //   // For each tier
  //   const staticDealPotentials = [0, 0, 0, 0, 0];

  //   for (const action of connects) {
  //     // Cache, same for each connection of tier
  //     if (!staticDealPotentials[action.connection.tier]) {
  //       staticDealPotentials[action.connection.tier] = evaluateDealPotential(
  //         state,
  //         action
  //       );
  //     }
  //     let s =
  //       staticDealPotentials[action.connection.tier] +
  //       evaluateConnectPotential(state, action.connection);
  //     if (s > biggestScore) {
  //       biggestAction = action;
  //       biggestScore = s;
  //     }
