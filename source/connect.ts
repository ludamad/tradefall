...
const priceQualityDistribution = gaussian(1.05, 0.1);
const dealChanceDistribution = gaussian(0.5, 0.02);

export function connectToString({
  name,
  tier,
  dealChance,
  sellsToPlayer,
  ...
  return `${buyerPrefix} ${name}: Tier ${tier + 1} Chance ${Math.round(
    dealChance * 100
  )}% Fairness ${Math.round(priceQuality * 100)}% Amount ${Math.round(
    dealSize * 100
  )}%`;
}
...
export function generateConnect(
  ...
  const priceQuality = clamp(priceQualityDistribution.ppf(seed), 0.9, 2);
  const dealChance = clamp(dealChanceDistribution.ppf(1 - seed), 0.25, 1.0);
  const dealSize = clamp(priceQualityDistribution.ppf(Math.random()), 0.5, 2);
...
export function dealToString(
  { name }: Connection,
  { daysLeft, kind, cost, amount }: PlayerDealAction
) {
  const moneyPrefix = kind === "buy" ? "-" : "+";
  const drugsPrefix = kind !== "buy" ? "-" : "+";
  const rate = cost / amount;
  return `${daysLeft} days ${moneyPrefix}$${cost.toFixed(
    2
  )} ${drugsPrefix}${amount.toFixed(1)}g from ${name} ($${rate.toFixed(2)}/g)`;
}

export function dealToStringBrute(
  { connections }: GameState,
  deal: PlayerDealAction
) {
...
export function onConnectDayStart(connection: Connection) {
  ...
  if (deals.length <= 0 && Math.random() < connection.dealChance) {
    ...
    const isExplodingOffer = Math.random() < 0.1;
    const daysLeft = isExplodingOffer ? 1 : rand([2, 3, 4]);
    const amountRange = isBuy
      ? getBuyRange(connection)
      : getSellRange(connection);
    const amount = randRange(amountRange[0], amountRange[1]);
    const priceQuality =
      connection.priceQuality *
      clamp(priceQualityDistribution.ppf(Math.random()), 0.9, 1.1);
